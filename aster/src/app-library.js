        ...
        OS.register(r.id,{title:r.title,description:r.description,category:r.category,icon:r.icon,color:r.color,custom:true,width:850,height:610,
            keywords:[r.title,r.description,r.category,r.publisher,r.version].join(' '),mount:async w=>{
                ...
                if(current.kind==='html') {
                    ...
                    const prepared=OS.lunaPrepareHTML?await OS.lunaPrepareHTML(html,current):{html,trusted:false};if(!alive||w.closed)return;
                    ...
                    OS.clipboardTools.attachFrame(frame,w);frame.srcdoc=OS.webIO?OS.webIO.bootstrap(prepared.html):prepared.html;w.body.append(frame);OS.webIO?.attach(w,frame,r.id,'about:srcdoc',true);
                }
