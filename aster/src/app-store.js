...
(() => {
    ...
    OS.register('store',{title:'App Center',description:'Discover web apps and manage your local app library.',category:'System',width:1060,height:740,minWidth:370,singleton:true,mount:async(w,options)=>{
        let view=options.mode||options.view||w.state.libraryView||'all', selected=options.app||null, query='',category='',order='name';
        ...
        const brand=el('div',{class:'library-brand'},el('span',{html:OS.appIcon('store',36)}),el('div',{},el('strong',{text:'App Store (App Center)'}),el('small',{text:'YOUR LOCAL LIBRARY'})));
        ...
        const labels={all:'Alle Apps',discover:'Katalog',installed:'Installiert',favorites:'Favoriten',builtin:'Aster-Programme'};
        ...
        function resultApps(){const all=[...OS.apps.values()].filter(a=>!a.hidden);return all.filter(a=>view==='all'?true:view==='discover'?a.webApp:view==='builtin'?!a.webApp&&!a.custom:view==='favorites'?lib.get(a.id)?.favorite:a.custom).map(a=>({...a,...(lib.get(a.id)||{})}));}
        ...
        function renderDetails(app){
            ...
            detail.append(el('p',{class:'library-safety',text:r?'Ordinary imported apps and web links stay in an opaque sandbox. Only SHA-256-verified bundled Luna originals receive their browser-storage context. HTML apps can request scoped Aster file access through the existing broker. Web links do not gain file access automatically. Publisher and version are self-reported.':app.webApp?'Mapped Luna catalog entries open the verified local original HTML. Other catalog apps load from their original reviewed project URLs. Trusted same-origin catalog apps retain their existing permissions; browser checks still apply.':'This app is supplied with Aster. Update Aster to update built-in apps.'}));
        ...
        function render(){
            ...
            const grid=el('div',{class:'library-grid'});for(const app of results){const r=lib.get(app.id),card=el('article',{class:'library-card','data-library-app':app.id});
                ...
                card.append(el('div',{class:'library-card-top'},el('span',{html:OS.appIcon(app.id,44)}),el('span',{class:'library-tag',text:app.localPackage?'Original · lokal':r?r.kind==='html'?'Original HTML':'Web link':app.webApp?'Katalog':'Aster'})),details,el('p',{class:'library-card-description',text:app.description||'No description added.'}),el('small',{text:app.category}));
